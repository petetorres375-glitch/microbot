...
Active portfolio: **ET, EPD, GOOG, SOFI** (swing) **+ PENG** (ORB, held overnight due to the stuck-order incident below — see "Intraday engine fix (2026-07-09, commit 38109f5)"). `MAX_OPEN_POSITIONS=8` — 5/8.
...
| **PENG** (ORB) | $68.70 | 12 shares. **Failed to close at the 2026-07-08 3:55 PM hard close** — the scale-out and EOD-close code kept losing track of the live stop order after a slow cancel, so it rode overnight in violation of the ORB "never overnight" rule. Protected only by a stale $70.45 GTC stop (no target leg). Currently ~$84.34, +$187.63 unrealized — up big by luck, not by design. Root cause fixed same day the incident was noticed (2026-07-09, commit `38109f5`); PENG itself was **left open, not force-closed**, per user decision to fix the bug separately from what to do with the position. |
