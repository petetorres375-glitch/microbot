...
5 16 * * 1-5 cd /home/lenovo-home/microbot && source .venv/bin/activate && python -u check_orb_gate.py >> /home/lenovo-home/microbot/orb_gate_check.log 2>&1
```
...
**Performance gate:** After 20+ trades, pull any strategy below 45% win rate. Track results in `intraday_trades` and `intraday_daily` journal tables. As of the 2026-09-01 STDN loss, ORB sits at 44.1% (59 closed trades, -$1,350.08 lifetime) — just below the 45% gate for the first time since the 2026-08-28 R-multiple fix; STDN's -$892.75 alone accounts for most of the day's swing. Watch the next several trades — one more good win rate stretch clears the line, another bad one and this strategy is a pull candidate per policy.
