...
### Walk-forward design caveat + OOS trade-count logging (2026-09-16, commit `910ad34`)

Surfaced by a structural question about the optimizer during a conversation, not a bug report: `optimizer.py`'s "walk-forward" is actually a **single static 75/25 chronological split per run** (`IS_SPLIT = 0.75`, `df.iloc[:cut]`/`df.iloc[cut:]`), not classic multi-fold walk-forward — there's real IS/OOS separation within one run (no leakage), but no rolling/disjoint folds. Because `LOOKBACK_DAYS=1100` and the optimizer runs weekly, consecutive weeks' OOS windows overlap by ~97% (only ~7 of ~275 OOS days are new each week) — so a proposal "validated OOS" one week and another "validated OOS" three weeks later are largely the same evidence re-tested, not independent confirmations. This is the most likely explanation for `dividend_momentum`'s `slow` parameter flip-flopping 80→100→80 across three weekly runs (see "Optimizer Params Aug 10" below) rather than settling once a real edge was found. The flat `MIN_IMPROVEMENT_PCT = 15.0` threshold also doesn't scale with how much selection pressure a given run applies (best-of-5-OOS-survivors of a 27–81-combo IS grid is a different multiple-comparisons problem for `trend_momentum` — 81 combos — than for a 3-param grid), and nothing checks whether a low-frequency strategy (`breakout_52w`, `ema_pullback`) is even getting a meaningful OOS trade count before its params get "improved."

Not restructured — that's a real design decision (bootstrap-derived threshold, disjoint folds with a cross-fold parameter-stability check, a minimum-OOS-trade-count floor before a strategy is even eligible) that changes which proposals get created/approved and needs a deliberate call, not a silent rewrite. What *was* shipped same day, as a zero-behavior-change diagnostic: `param_proposals` now logs `oos_trades`/`current_oos_trades` alongside the score and improvement %, so "beat baseline by 20%" is no longer ambiguous between 4 trades and 40 — `_universe_score()` returns `(score, trade_count)` instead of just `score`, threaded through `_optimize_one()` and printed/persisted; `python -m microbot.approvals --params` shows both counts now. This doesn't change which proposals get created, but it will make it possible to retroactively judge how much to trust the already-approved 2026-07-13 through 2026-08-10 proposals (see "Optimizer Params Jul 13"/"Optimizer Params Jul 20"/"Optimizer Params Aug 10" below) once new proposals start carrying real counts — the historical ones predate this field and can't be backfilled without re-running the exact same-dated backtest split.

**Decision (2026-09-16): wait, then set the trade-count floor.** User chose to let the new `oos_trades`/`current_oos_trades` logging accumulate real numbers across a few weekly optimizer runs (~3-4 weeks, so roughly early-to-mid October 2026) before picking a minimum-OOS-trade-count eligibility floor — rather than guessing a number now. The real trigger is having enough actual weekly-run data points to see how thin `breakout_52w`/`ema_pullback`'s OOS samples genuinely are before setting a threshold that would silence them, not the calendar date itself. The bootstrap-derived-threshold and disjoint-fold/parameter-stability ideas are still open and separate from this — not committed to either way.
...
**Not yet verified live** — the fix takes effect on the next trading day's 9:34/9:35 AM cron; watch for `breakout` finally appearing in `engine.log`'s live-signal bullets or veto lines instead of going silently dark. Still unverified as of 2026-08-31 (0/1 trading day since the fix — all 7 signals that morning were `trend_momentum`).

**Verification automated 2026-08-31 (commit `c34dce5`):** `check_breakout_fix.sh`, cron `45 9 * * 1-5`, checks each day's `engine.log` for `breakout`/`breakout_52w` in a live-signal bullet or `vetoed by analyzer:` line. A first version (one-time cron for 8/31) had two bugs, both fixed before this: (1) the crontab line was `bash check_breakout_fix.sh` with no leading `cd`, so cron (CWD = home dir, not the repo) couldn't find the script at all and silently discarded the error (no MTA installed) — every crontab line invoking a script needs the `cd /home/lenovo-home/microbot &&` prefix, the same category of silent cron failure as the `SHELL=/bin/bash` gotcha above (cron's minimal environment breaking an assumption the script only holds when run interactively) though not the identical bug; (2) the matcher grepped the word "breakout" anywhere in the day's log section, which false-positived daily on the `using promoted params for: ...breakout...` line and the cumulative performance-report table row — neither means breakout fired *today*. Fixed to only match live-signal bullets/veto lines. Self-deletes (its own crontab line + the script file) the day it finds real evidence, so no manual re-scheduling loop is needed. Known blind spot, documented in the script itself: the "N Signals Today" bullets are top-3-only and a signal on an already-held symbol is silently dropped (see the Notify Rankings Bug note below) — a breakout signal hiding in either gap won't be caught by this check.

**Verified live 2026-09-01:** PFE fired as a live `breakout` signal (`AUTO(CLEAN) 423x PFE @~29.02 stop 27.84 target 31.4`) and auto-executed — `check_breakout_fix.sh` found it, logged the match to `breakout_fix_check.log`, and self-deleted its own crontab line and script file as designed. The fix works: pace-adjusted volume let `breakout` clear its same-day volume gate for the first time since the drought began. `breakout` produced a second live signal the very next day (2026-09-02: VALE, also auto-executed CLEAN) — both PFE and VALE are open swing positions as of this writing. `breakout_fix_check.log` (kept, not gitignored) has the full before/after evidence trail.
...
## Market-data retry resilience added 2026-08-28 (commit pending)

Surfaced by the same Alpaca-skills comparison as the order-idempotency fix above (this time from `rate-limits-resilience`, a Broker API skill whose general pattern still applied). `MarketData.bars()` in `microbot/data.py` made a single `get_stock_bars()` call with no retry of its own. alpaca-py's REST client already retries `429`/`504` HTTP *responses* automatically (3 attempts, 3s apart — `alpaca.common.rest.RESTClient`'s `DEFAULT_RETRY_EXCEPTION_CODES`), but that only covers responses that actually came back — a raw connection timeout raises inside `requests.Session.request()` before there's a response to inspect, so it propagates with zero retry attempted anywhere. `500`/`502`/`503` also fall outside alpaca-py's default retry set (only `504` is covered). This directly matches the documented "Alpaca paper API reliably congests 9:30–9:50 AM ET" incidents (`request timed out` errors) — exactly the window the 9:34/9:35/9:41 AM crons scan 40+ symbols in, and `screener.py`'s per-symbol `except Exception: skip` silently dropped a symbol's candidacy for the day on any such transient blip, indistinguishable from a genuinely bad symbol.

Added `data._get_stock_bars_with_retry()` — 3 attempts, linear backoff, retries `ConnectionError`/`Timeout` and 5xx `APIError`s (`500`/`502`/`503`), never retries a real 4xx (bad symbol, bad auth — retrying won't fix those). `MarketData.bars()` now calls it instead of `self.client.get_stock_bars()` directly. New tests in `tests/test_data_retry.py`; full suite (62 tests) passes; verified against a real live Alpaca call (AAPL, 30-day lookback) to confirm normal fetches are unaffected.
...
**ORB win-rate gate re-check automated 2026-09-01 (commit `b341e35`, extended `0d6d61a`):** `check_orb_gate.py`, cron `5 16 * * 1-5` (weekdays 4:05 PM ET, after the day's trail/reconcile cadence), same self-deleting pattern as `check_breakout_fix.sh` — counts ORB trades **closed after 2026-09-01** (the too-tight-range fix date, see below) so the STDN loss that bug caused doesn't get counted against a fix meant to prevent it. Once 15 post-fix trades have closed, evaluates their win rate (≥45% PASS, <45% FAIL) and **always** fires a desktop notification via `microbot.notify.notify()` with the gate verdict plus a "live-strategies-only" trend: swing P&L excluding the permanently paused/stopped `ema_pullback`/`manual` strategies, alongside ORB's full lifetime P&L — added per user request 2026-09-01 after they said they'd reconsider rebuilding the bot without seeing improvement, so the eventual decision is made off that trend line rather than the noisier all-time total (which still carries GOOG/AMD/`ema_pullback`-era losses from things already cut). Sanity check at commit time: swing ex-dead-strategies is **+$84.87 over 37 trades** (full swing lifetime is -$310.59 — the dead strategies account for the entire deficit and then some) vs. ORB's -$1,350.08/59 trades — confirms the swing side is the healthier one; ORB is what this gate is actually re-testing. Self-deletes (crontab line + file) once the sample completes, either verdict. Before that it just logs progress (`N/15 post-fix trades`) and exits quietly. Not version-controlled (crontab isn't tracked in git) — noted here so a crontab rebuild doesn't silently drop it. **Watch `orb_gate_check.log`** for the eventual verdict + trend.

**Too-tight ORB range fix 2026-09-01 (commit `2c0ae69`):** triggered by the user checking on account P&L and pushing back on "well in the negative" — digging into today's numbers found the worst single ORB loss on record. STDN entered on a range=0.08 opening bar (high=16.24, low=16.16 — **0.49% of price**, not literally zero so the 2026-08-06 zero-width guard let it through), 3571 shares @ $16.30, stopped out at $16.05 (stop was $16.16) for **-$892.75, -1.79R**. Root cause: the $0.11 stop-market slippage was ordinary/typical in absolute terms (other symbols see similar per-share slippage on stop-outs), but because the risk-per-share was only $0.14, that same $0.11 slip consumed 79% of the intended risk *on top of* the full 1R already used reaching the stop — and because sizing targets a fixed dollar risk budget, the tiny risk-per-share also ballooned share count (3571 shares, among the largest ORB position sizes ever), so the same-magnitude slippage produced an outsized dollar loss too. Checked the full range% distribution across all past ORB entries: STDN (0.49%), AEVA (0.40%), and QURE (0.18%) are the only entered trades below 0.5% of price, and all three slipped worse than -1R (QURE -1.12R); everything else in the entered-trade history sits at 1%+ with normal-looking outcomes. Fixed: `_set_orb()`'s `orb_invalid` guard (see PRCT fix below) now also rejects any range below `MIN_ORB_RANGE_PCT` (0.5% of price), not just literal zero-width — same rationale generalized, a range that tight isn't real support/resistance either way. 2 new tests in `tests/test_intraday_orb.py`; full suite (64 tests) passes.
...
**Performance gate:** After 20+ trades, pull any strategy below 45% win rate. Track results in `intraday_trades` and `intraday_daily` journal tables. **Gate tripped 2026-09-22 — ORB paused.** See "ORB gate FAILED, ORB paused 2026-09-22" below for the final numbers and the crontab change.

**Phase 1 "prove it" gate re-checked 2026-09-10 (the scheduled date, set 2026-07-30 for ~6 weeks out) — still not met.** Gate requires 20+ ORB trades at ≥45% WR and 20+ swing trades with positive expectancy. ORB: 64 lifetime trades, 43.8% WR — below 45% at all three check-ins now (7/30, 8/17, 9/10) despite the R-multiple fix (8/28) and too-tight-range fix (9/1). The post-fix clean sample (`check_orb_gate.py`'s 15-trade cohort, trades closed after 2026-09-01) was 5/15 at check-in time. Swing: 53 lifetime trades, -$809.80; ex-dead-strategy (excl. `ema_pullback`/`manual`) -$414.34/39 trades, 33% WR — worse than the 8/17 snapshot, not better. `breakout` is the worst swing $ performer (-$571.93/12) but only 1 trade (PFE, -$456.43/-0.91R, closed 9/8) has happened under its current promoted params (channel=15/vol_mult=1.2/stop_mult=2.0, live since 8/3) — the rest predate that retune.

Decision made 2026-09-10 rather than pausing either strategy outright: (1) **ORB** — run to the existing 15-trade post-fix checkpoint above instead of pausing pre-emptively; that checkpoint (not today's calendar date) is the real decision point. No code/crontab change. (2) **`breakout`** — ran `python run_optimizer.py --strategy breakout` (scoped to just this strategy) before deciding whether to pull it. Result: **0 proposals — the walk-forward grid search's best combo exactly matched the current live params** (`proposed OOS=21.193 current OOS=21.193 Δ=+0.0%`). Unlike `ema_pullback`'s July optimizer run (every combo scored 0.000, no traction at all), this run did get traction and confirmed channel=15/vol_mult=1.2/stop_mult=2.0 is already the best-scoring combo in its grid — so `breakout`'s problem isn't tunable params, it's either a genuinely weak edge or (more likely, given n=1) just too thin a live sample to judge yet. Holding off on pausing it; the next real look is at 5+ live trades under current params (same bar used for `trend_momentum` after its 7/13 retune) or whenever the ORB gate above fires, whichever comes first.

**`breakout` update 2026-09-14 — now 3 of the promised 5 live trades, all losers.** NOK and VALE (both entered under the current channel=15/vol_mult=1.2/stop_mult=2.0 params) stopped out the same morning: NOK -$553.32 (-1.11R) and VALE -$525.13 (-1.05R), both at 9:36 AM via the trail-ratchet ordinary stop, not a bug. Combined with PFE (-$456.43, closed 9/8), that's **3/3 losing trades since the 8/3 retune, -$1,534.88 total** — 0% win rate on the exact sample size (5 live trades) that was set as the next decision checkpoint on 2026-09-10. Two more live trades will complete that checkpoint; at 0-for-3 already, a 4th loss would put this strategy in clear pull territory before the count even reaches 5. Not paused yet — holding to the pre-agreed checkpoint rather than reacting to two bad trades in one morning — but this is the closest `breakout` has come to the pull threshold since it was retuned.

**`breakout` update 2026-09-16 — 4th of 5 checkpoint trades, also a loser: 4/4, 0% win rate.** HOOD (entered under the same channel=15/vol_mult=1.2/stop_mult=2.0 params) stopped out 2026-09-16 11:32 AM ET at $104.81 (stop was $104.84, ~$0.03 ordinary slippage), entry $118.22, 39 shares → -$522.99 (-1.00R). Combined with PFE/NOK/VALE, that's **4/4 losing trades since the 8/3 retune, -$2,057.87 total, 0% win rate** — one trade short of the pre-agreed 5-trade checkpoint from 2026-09-10, but already unambiguous: no winners at all across the full post-retune sample so far. Flagged to the user same day; decided same day to let it play out to the 5th trade rather than pause early — holding to the pre-agreed checkpoint.

**ORB gate FAILED, ORB paused 2026-09-22.** `check_orb_gate.py`'s 15-trade post-fix checkpoint (trades closed after the 2026-09-01 too-tight-range fix) completed today — WOLF (-$941.12, -1.3R, on a range just above the 0.5% MIN_ORB_RANGE_PCT floor) and GRAL (-$585.90, -1.0R) pushed the count from 14 to 16, tripping the daily loss limit and halting for the day (-$1,527.02). Final sample: **16 trades, 6W/10L, 37.5% win rate, -$4,426.38 P&L** — below the 45% gate even excluding the pre-fix-era bug. The self-deleting gate cron fired its verdict ("Per policy, consider pulling ORB") and removed itself from crontab as designed. Live-strategies-only trend requested back on 9/1: 119 combined trades, 38.7% WR, -$8,293.06 (swing ex-dead-strategies -$2,516.60/44 trades, ORB lifetime -$5,776.46/75 trades) — swing is the less-bad side but still net negative.

Per the [[feedback_rebuild_threshold_sep01]] policy (hold course until a checkpoint fails, then act on real numbers, no spin), user directed pausing ORB same day. **Action taken:** removed the two ORB-related crontab lines (`34 9 * * 1-5 ... run_intraday.py` and `15 9 * * 1-5 ... microbot.intraday_scanner`) — no code deleted, `microbot/intraday_scanner.py`/`run_intraday.py` untouched, easily resumed by re-adding the cron lines. The CCR "Intraday pre-market scanner" routine (`trig_01TX4CDGSGMLscLLgtkgeKAr`, 9:15 AM ET) is still enabled but harmless — it only prints a news briefing and can't reach Alpaca from the sandbox regardless; left as-is since disabling it requires the routines UI, not code. No open ORB positions existed at pause time (ORB hard-closes everything by 3:55 PM daily, so there's never overnight ORB risk to unwind). `breakout`'s swing checkpoint (separate from ORB) is unaffected and still open at 4/4 losers, one trade short of its own 5-trade decision point.

**Replacement options for the intraday layer, not yet decided:**
1. **Leave it paused, don't replace.** Swing is the working half of the bot (`dividend_momentum` 75% WR, +$146; rsi2_reversion positive) — simplest path is to stop trying to make intraday work and put research effort into swing strategies instead.
2. **Low-float bull flag** — already the next strategy on [[project_strategy_roadmap]], predates this gate failure. Different setup (continuation flag after a strong initial move) than ORB's opening-range breakout; would need its own backtest/paper validation before going live, not a drop-in swap.
3. **Redesign ORB itself** — the two fixes shipped (R-multiple bug 8/28, too-tight-range 9/1) fixed real measurement/entry bugs but didn't move the win rate past 45%; a genuine redesign (different range window, added trend/volume confirmation filter, VWAP-based entry instead of pure Donchian-style opening range) is a bigger lift than a parameter retune and hasn't been attempted.
4. **Retune ORB's risk/reward instead of the entry logic** — current scale-out/trail structure (2:1 partial, 25%-of-gain trail) was tuned once (2026-06-17) and never revisited against the full post-fix data; expectancy is close to breakeven (-0.08R lifetime pre-STDN) so a R:R tweak might flip it positive without touching entries at all.

No option chosen yet — flagged for the next session/discussion.

## Swing strategy win-rate/R-expectancy review (2026-09-22, commits `acf984a`, `6260482`)

Prompted by the user asking for R-expectancy to be raised without sacrificing win rate, with an explicit goal of weighting the system toward mean reversion. Worked through a 7-task plan, one step at a time with tests at every step (63 new tests across 6 new analysis modules; full suite 113 tests passes). Full detail lives in the modules themselves (`microbot/rebaseline.py`, `rsi2_variants.py`, `mean_reversion_variants.py`, `filter_tests.py`, `momentum_filter_tests.py`, `overfitting_check.py`) — this is the summary.

**Task 1 (re-baseline every strategy in R terms):** all strategies backtest positive full-sample (+0.24R to +0.30R), except `mean_reversion` which produced **zero trades** — a real finding, not a bug (see Task 3).

**Task 2 (rsi2_reversion vs Connors' actual published exit):** verified Connors' real rule from source (StockCharts ChartSchool: exit on close > 5-day SMA, no stop) rather than relying on memory. Tested it head-to-head against the current 1:1 bracket at the same stop width for a fair comparison. Verdict: **keep the current bracket.** Connors' exit wins on win rate (67% vs 63%) but loses badly on expectancy (+0.06R vs +0.26R) — exactly the "higher win rate, lower expectancy" regression the user asked to watch for. No change made.

**Task 3 (mean_reversion, same treatment):** found the 2026-06-21 weekly trend filter mechanically zeroes out `mean_reversion` — an oversold daily dip and a rising weekly EMA are nearly mutually exclusive for this setup (6 raw candidate days across the whole universe, all killed by the filter). Disabling the weekly filter (backtest-only, not live) got to 39 trades, but expectancy was **negative** (-0.125R) — worse than the misleading n=5 result before that. The higher-low gate turned out to be doing real work (removing it is what let the negative trades in), consistent with its original purpose of blocking falling-knife entries. Connors-style exit-on-strength looked good unstopped (+0.113R, 60.5% WR) but carried serious tail risk (-4.12R worst trade) that mostly vanished the moment a normal stop was added back (+0.024R). **No viable configuration found for `mean_reversion` in any test; not changed, not promoted.**

**Task 4 (market-regime + volatility filters):** built a SPY-derived market-regime filter (close > SPY's 200-day SMA) and a volatility filter (20-day realized-vol proxy, top-25%-by-expanding-percentile = "elevated" — no live VIX feed available). Both filters, alone or combined, were a **clean regression for `rsi2_reversion`** (lower win rate, lower expectancy, fewer trades, no trade-off benefit) and didn't rescue `mean_reversion` either.

**Task 5 (overfitting check on rsi2_reversion):** chronological 75/25 in-sample/out-of-sample split (matching `optimizer.py`'s own `IS_SPLIT` convention) revealed the flashy full-sample +0.260R is mostly front-loaded — real out-of-sample expectancy is just **+0.029R** (80 trades, 52.5% WR, down from 67.1% IS). Still positive, didn't flip sign, but nowhere near as strong as the headline number implied. A 3×3 parameter-sensitivity sweep around the live params (rsi_buy=10, stop_mult=3.0) showed the edge is NOT a narrow overfit spike — all 9 nearby combos scored positive (+0.19R to +0.30R) — so this looks like a real but weaker-than-advertised effect, not curve-fitting.

**Task 6 (momentum strategies, filters only, no exit changes per instruction):** added a per-symbol trend filter (close > that stock's own 200-day SMA) via a generic wrapper, tested against `trend_momentum`, `breakout`, `breakout_52w`. Initial full-sample read looked great for both `trend_momentum` (+0.242R→+0.332R) and `breakout` (+0.297R→+0.330R) — but the same IS/OOS check from Task 5, applied here too, showed they behave very differently: `breakout`'s improvement **held up OOS** (+0.245R unfiltered → +0.274R filtered, both on the untouched holdout), while `trend_momentum`'s **collapsed** (+0.293R unfiltered IS → +0.084R unfiltered OOS; +0.413R filtered IS → +0.129R filtered OOS — worse than doing nothing). The trend filter isn't the problem for `trend_momentum` — the strategy's current live params just don't hold up going forward regardless of filtering, a separate issue from this review. `breakout_52w`'s trend filter was a no-op (a 200-day-high entry already implies being above the 200-day SMA in every case tested) and its other filters only hurt it.
...
Follow-up fixes the same day: (1) ~~no time window~~ — **fixed:** only trades *entered* under a strategy's current params count (`active_params.promoted_ts`, via new `journal.fetch_promoted_dates()`). Entry time comes from the matching `orders` row, since `trades` only stores close time — so e.g. SOFI (entered 7/8, closed 7/13) doesn't count toward post-7/13 `trend_momentum`. Strategies never promoted by the optimizer keep their full history. Every approved optimizer proposal resets that strategy's count to zero. Live effect: `trend_momentum` (3 trades since 7/13) and `breakout` (4 since 8/3) dropped below the 6-trade minimum and were unvetoed; only `ema_pullback`/`manual` remain vetoed. Because that would have let `breakout` (**0W/4L, -1.03R since its 8/3 retune**) trade again at ~$500 risk, the user chose to **pause `breakout` via `DISABLED_STRATEGIES=ema_pullback,breakout` in `.env`** (local-only, gitignored) — it stays off until a separate decision; the 9/16 "play out to the 5th checkpoint trade" plan is superseded (the old veto had silently blocked that 5th trade since 9/23 anyway). (2) ~~`rsi2_reversion` lockout~~ — **fixed same day:** a negative average alone no longer vetoes. A 1:1 strategy with a real 60% win rate still shows avg R <= 0 after 6 trades ~46% of the time (61% at rsi2's 52.5% OOS win rate), so a veto now needs avg R + 1 standard error < 0 (~84% confident it's really losing; `FeedbackConfig.z`). Current veto list unchanged by this (all four have upper bound < 0). rsi2 from its 1W/3L record: next two 1W/1L -> stays active (old rule vetoed it); two losers (1W/5L) -> still vetoed, as it should be.
...
**Verdict-routine sync (fixed same day, before any approval):** approved symbols live in `microbot.db` (gitignored), but the 8:30 AM CCR verdicts routine builds its universe from git — so an approved symbol would have been scanned but never gotten a verdict, silently skipped forever with `verdict=none — CLEAN required` (same bug class as the 2026-07-14 universe/verdicts sync fix). Fix: approving writes `approved_universe.txt` (tracked, repo root, regenerated from the DB minus `UNIVERSE_EXCLUSIONS`) and commits + pushes **only that file** (`git commit -- approved_universe.txt`) right away, so the next morning's routine sees it. The Morning Signal Analysis routine (`trig_019TFaNMJyiH1atY2kykNHGD`) Step 1 was updated 2026-09-22 to also read that file. If the push fails, the approval CLI says so — the symbol is approved locally but gets no verdicts until the file is pushed. Dropping a symbol via `UNIVERSE_EXCLUSIONS` stops the scan immediately; the file only refreshes on the next approval (a stale entry there just costs one extra verdict lookup, it can't cause a trade).
...
**Rollout check automated:** `check_discovery_rollout.py` (untracked, one-shot cron `12 16 23 9 *`) runs after close 2026-09-23 — verifies BFLY/NU got morning verdicts, engine/research runs finished without tracebacks, and trail ran 8/8 clean after the date fix. Result goes to `discovery_rollout_check.log` + a desktop pop-up; then it removes its own crontab line and file (same pattern as `check_breakout_fix.sh`). Dry-run against 2026-09-22 logs behaved as expected (verdicts FAIL only because BFLY/NU were approved after that morning's run; engine/research/trail OK).
...
**`breakout` paused 2026-09-25** via `DISABLED_STRATEGIES=ema_pullback,breakout` in `.env` — 0W/4L since its 8/3 retune; see "Analyzer veto switched from $ to R" above. To resume: remove `breakout` from that line.
...
DISABLED_STRATEGIES=ema_pullback,breakout   # paused strategies (breakout since 2026-09-25)
