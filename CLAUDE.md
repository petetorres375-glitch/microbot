...
| Symbol | Entry | Notes |
|---|---|---|
| ET | $19.45 | Energy Transfer. Dividend momentum. 72 shares. Stop $18.76, target $20.84. Engine auto-entry 2026-07-06 (CLEAN verdict). Currently ~$19.90, +$30.96. |
| EPD | $37.72 | Enterprise Products Partners. Dividend momentum. 36 shares. Stop $36.36, target $40.44. Engine auto-entry 2026-07-08. Currently ~$37.82, +$2.34. |
| GOOG | $358.23 | Google. EMA pullback re-entry 2026-07-01 (prior entry stopped Jun 22 at −1R). 3 shares. Stop $341.60, target $391.49 (bracket). Currently ~$355.21, −$12.36. |
| SOFI | $17.49 | SoFi. Trend momentum. 36 shares. Stop $16.11, target $20.26. Engine auto-entry 2026-07-08. Currently ~$17.80, +$12.60. |
| **PENG** (ORB-origin) | $68.70 | 12 shares. **Failed to close at the 2026-07-08 3:55 PM hard close** — the scale-out and EOD-close code kept losing track of the live stop order after a slow cancel, so it rode overnight in violation of the ORB "never overnight" rule. Root cause fixed same day (commit `38109f5`); PENG itself was left open, not force-closed, per user decision to separate the bug fix from the position decision. Since neither `trail.py` (no matching swing journal row) nor tomorrow's intraday cron (states rebuilt fresh from each day's scan candidates, not existing positions) would ever manage it again, it was **manually adopted into the swing journal** 2026-07-09 (`journal.log_manual_order(..., strategy="orb_manual")`, original entry $68.70 / stop $64.29 / target $76.17 preserved for the R-multiple denominator) so `trail.py` now ratchets it like any other swing position. Confirmed working same day: first `trail.py` run after adoption ratcheted the live stop 72.86 → 77.04 (+3.8R). |

CCL (closed 2026-07-09 9:36 AM ET via manual market sell at $26.31, 25 shares, entry $28.60 — cleanup from a related stuck-order incident that paused the whole crontab 2026-07-08 4:47 PM ET) and CSCO (stopped out 2026-07-07 at $111.70, 8 shares, entry $117.88, ~−$41 net) are no longer held.

**Crontab was paused 2026-07-08 4:47 PM ET** ("CCL stuck-order incident") and manually restored 2026-07-09 ~9:45 AM ET once the underlying bug was understood and fixed — see commit `38109f5`. Today's 9:34/9:35/9:36/9:41 AM slots had already passed by the time it was restored, so full automation resumes tomorrow; today's remaining interval jobs (hourly `trail.py`/`watch_spcx.py`, 10-minute `check_positions.py`) still fire.

**Note for future ORB positions that survive past their entry day:** `run_intraday.py`'s `IntradayEngine.states` dict is rebuilt fresh each run from that day's scanner candidates (see `run()`), not from existing Alpaca positions — so any ORB position still open the next trading day is invisible to the intraday engine going forward. `trail.py` also won't see it unless it has a row in the swing journal's `orders` table. If this happens again, use `journal.log_manual_order(alpaca_id, symbol, qty, entry, stop, target, strategy="orb_manual")` with the *original* ORB entry/stop/target (not the current live trailed stop) to adopt it into `trail.py`'s management, the same way PENG was handled here.
