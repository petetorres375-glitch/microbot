...
**ORB win-rate gate re-check automated 2026-09-01 (commit `b341e35`):** `check_orb_gate.py`, cron `5 16 * * 1-5` (weekdays 4:05 PM ET, after the day's trail/reconcile cadence), same self-deleting pattern as `check_breakout_fix.sh` — counts ORB trades **closed after 2026-09-01** (the too-tight-range fix date, see below) so the STDN loss that bug caused doesn't get counted against a fix meant to prevent it. Once 15 post-fix trades have closed, evaluates their win rate: ≥45% logs PASS and self-deletes quietly; <45% logs FAIL, fires a critical desktop notification via `microbot.notify.notify()` recommending ORB be pulled per the documented gate policy, then self-deletes. Before the sample completes it just logs progress (`N/15 post-fix trades`) and exits. Not version-controlled (crontab isn't tracked in git) — noted here so a crontab rebuild doesn't silently drop it. **Watch `orb_gate_check.log`** for the eventual PASS/FAIL verdict.
